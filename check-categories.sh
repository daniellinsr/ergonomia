...
docker exec -i $POSTGRES_CONTAINER psql -U "$DB_USER" -d "$DB_NAME" -c "SELECT DISTINCT categoria, COUNT(*) as total FROM perigos_catalogo GROUP BY categoria ORDER BY categoria;"

echo ""
echo -e "${YELLOW}📊 Detalhamento por número:${NC}"
docker exec -i $POSTGRES_CONTAINER psql -U "$DB_USER" -d "$DB_NAME" -c "SELECT categoria, MIN(numero) as primeiro, MAX(numero) as ultimo, COUNT(*) as quantidade FROM perigos_catalogo GROUP BY categoria ORDER BY MIN(numero);"
